...
> **Version: v1.0**
> **Created: 2026-06-28**
> **Status: ACTIVE** (replaces all prior versions)
...
### Features:
- **Click 🔊** → speaks the word (Web Speech API, female voice, rate 0.88, pitch 1.05)
- **Hover 🔊** → popup shows IPA phonetic + Chinese meaning
- **Inline terms** → dotted blue underline + small 🔊 button
- **Female voice** → priority selection: Google US English > Microsoft Natural > macOS Samantha > fallback
...
2. **Answer input** — textarea (min-height 80px), placeholder in EN+中文
3. **Submit button** — "📤 Submit · 提交"
4. **Reset button** — "🔄 Retry · 重做"
5. **Model answer** — hidden until submit, contains:
...
6. **Feedback panel** — three columns:
...
7. **Answer log** (page bottom, dark themed) — tracks all attempts with score/time
...
- [ ] Female voice selection code present
...
|---------|------|---------|
| **v1.0** | 2026-06-28 | Initial SOP created. Consolidates all requirements from Unit TS1 development: bilingual annotations, diagram extraction, pronunciation widget (female voice), interactive quiz engine, language analysis, IGCSE syllabus verification. |
